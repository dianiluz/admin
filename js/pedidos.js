...
// --- PARCHE DE SEGURIDAD PARA TOASTS CONGELADAS ---
window.mostrarToast = function(mensaje, tipo = 'exito') {
    ...
    // Eliminación forzada segura independiente de animaciones CSS
    setTimeout(() => {
...
// --- SECCIÓN ARREGLADA: CIERRE FORZADO DE LA X Y CANCELAR ---
window.crearProductoDesdeERP = function(nombreInicial) {
    if (typeof window.abrirModalProducto === 'function') {
        ...
        setTimeout(() => {
            ...
            // Tomamos todos los modales generados y nos aseguramos de usar el que está visible
            const modales = document.querySelectorAll('#modal-producto');
            const modalProd = modales[modales.length - 1]; 
            
            if (modalProd) {
                modalProd.style.zIndex = '99999'; // Lo mandamos al frente de todo
                
                // Forzamos el evento en los botones de cerrar
...
window.renderPedidos = function() {
    ...
    const filtrarPedidos = () => {
        ...
        const filtrados = window.pedidosGlobales.filter(p => {
            const idPed = String(p.IDpedido || p.idpedido || "");
...
async function cargarUsuarios() {
    try {
        ...
        if (!error && data) window.usuariosGlobales = data;
    } catch(e) { console.warn("Fallo al pre-cargar usuarios", e); }
}
...
async function cargarPedidos() {
    ...
    if (!tbody) return;
    try {
        const { data, error } = await window.supabase.from('pedidos').select('*').order('fecha_creacion', { ascending: false });
        if (error) throw error;
        
        // Mapeo seguro IDpedido
        window.pedidosGlobales = (data || []).map(p => ({ ...p, IDpedido: p.idpedido || p.IDpedido }));
        renderizarTablaPedidos(window.pedidosGlobales);
    } catch (error) {
        console.error("Error pedidos:", error);
        tbody.innerHTML = `<tr><td colspan="7" style="color:red; text-align:center;">Error al cargar datos de Supabase.</td></tr>`;
    }
}
...
function renderizarTablaPedidos(pedidos) {
    ...
    pedidos.forEach(ped => {
        ...
        // Fechas corregidas a los nombres reales de la BD
...
window.abrirModalGestionPedido = async function(idPedido) {
    ...
    // Carga de productos del pedido desde BD usando el id exacto
    const { data: productosDB } = await window.supabase.from('pedidos_productos').select('*').eq('idpedido', idPedido);
    const productosPedido = productosDB || [];
    ...
    if (productosPedido.length > 0) {
        productosPedido.forEach(prod => {
            ...
            // --- MAGIA AÑADIDA AQUÍ: Si en la DB el nombre está nulo, lo busca en el catálogo
            let nombreProd = prod.producto;
            if (!nombreProd || nombreProd === 'NULL' || nombreProd.trim() === '') {
                const productoCatalogo = window.productosGlobales.find(p => p.ref === prod.ref_producto);
                if (productoCatalogo) {
                    nombreProd = productoCatalogo.producto || productoCatalogo['*producto'] || 'Producto Personalizado';
                } else {
                    nombreProd = 'Producto Personalizado';
                }
            }
        ...
        htmlProductos = '<tr><td colspan="4" style="text-align:center;">No hay productos registrados.</td></tr>';
    }
            ...
            <div class="modal-content" style="max-width:900px; max-height: 95vh; overflow-y:auto;">
                ...
                <div class="detalle-pedido-grid">
                    <div class="detalle-seccion">
                        ...
                        <div style="margin-top: 15px; padding: 10px; background: rgba(16, 185, 129, 0.1); border-radius: 4px; border: 1px solid #10b981;">
                            ...
                            <div class="drop-zone" id="drop-zone-soporte" style="margin-top: 10px; padding: 15px; text-align:center; border:2px dashed #10b981; background:#fff; cursor:pointer; border-radius:8px;">
                                <p style="color: var(--color-texto-suave); font-size: 12px; margin:0;">📸 Arrastra soporte de pago aquí o haz clic para subir</p>
                    ...
                    <div class="detalle-seccion">
                        ...
                        <form id="form-actualizar-pedido" style="display:flex; flex-direction:column; gap:10px;">
                            ...
                            <div><label style="font-size:12px; font-weight:600;">Guía / Rastreo (Solo NÚMEROS)</label><input type="number" id="gestion-guia" value="${pedido.guia || ''}" style="width:100%; padding:8px; border-radius:4px; border:1px solid #ccc;"></div>
                ...
                <div class="detalle-seccion">
                    ...
                    <table class="tabla-productos-pedido" style="width:100%;">
                        <thead style="background:#fce7f3; text-align:left;"><tr><th style="padding:8px;">Referencia</th><th>Producto</th><th>Cant</th><th>V. Unitario</th></tr></thead>
    ...
    document.getElementById('btn-eliminar-pedido').addEventListener('click', async () => {
        if (confirm(`¿Estás 100% segura de eliminar el pedido ${pedido.IDpedido}? Esta acción no se puede deshacer.`)) {
            try {
                await window.supabase.from('pedidos_productos').delete().eq('idpedido', pedido.IDpedido);
                await window.supabase.from('pedidos').delete().eq('idpedido', pedido.IDpedido);
    ...
    dropZone.addEventListener('dragover', (e) => { e.preventDefault(); dropZone.style.borderColor = '#db137a'; });
    dropZone.addEventListener('dragleave', () => dropZone.style.borderColor = '#10b981');
    dropZone.addEventListener('drop', (e) => { 
        e.preventDefault(); dropZone.style.borderColor = '#10b981'; 
        if (e.dataTransfer.files.length > 0) procesarArchivoSoporte(e.dataTransfer.files[0]); 
    });
    fileInput.addEventListener('change', (e) => { 
        if (e.target.files.length > 0) procesarArchivoSoporte(e.target.files[0]); 
    });

    function procesarArchivoSoporte(file) {
        document.getElementById('check-pago-confirmado').checked = true;
        previewContainer.innerHTML = `<img src="${URL.createObjectURL(file)}" style="max-height:100px; border-radius:4px; margin-top:5px;"> <br> ✅ ${file.name}`; 
    }
    ...
    document.getElementById('form-actualizar-pedido').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (error.message && error.message.includes('schema cache')) {
                alert("Supabase requiere actualizar su caché de columnas.\nPor favor, actualiza la página o espera 1 minuto e intenta de nuevo.");
            } else {
...
window.abrirModalCrearPedido = async function() {
    ...
    if (!window.usuariosGlobales || window.usuariosGlobales.length === 0) await cargarUsuarios();

    const modalHtml = `
        <div class="modal-overlay" id="modal-crear-pedido">
            <div class="modal-content" style="max-width: 1100px; height: 95vh; overflow-y:auto;">
    ...
    // BUSCADOR CLIENTES CON SUPABASE 
        ...
        if (matches.length > 0) {
            matches.forEach(m => {
                ...
                div.onclick = () => {
                    ...
                    buscadorCliente.value = m.nombre;
                };
        ...
        buscadorProd.addEventListener('input', (e) => {
            ...
            if(matches.length > 0) {
                matches.forEach(m => {
                    const div = document.createElement('div');
                    div.style.padding = '8px'; div.style.cursor = 'pointer'; div.style.borderBottom = '1px solid #eee';
                    div.innerHTML = `<strong>${m.ref}</strong> - ${m.producto || m.nombre}<br><small style="color:var(--color-primario);">$${Number(m.precio_base || 0).toLocaleString('es-CO')}</small>`;
                        ...
                        if (window.variacionesGlobales) {
                            window.variacionesGlobales.forEach(vg => {
                                if (window.productoCumpleCondicion(m, vg.columna, vg.valor)) {
                                    ...
                                    opt.value = inc; opt.setAttribute('data-name', textoMostrar);
                                    opt.textContent = `${vg.columna.split('|').pop()} (${textoMostrar}) (+ $${inc.toLocaleString('es-CO')})`;
                                    selectVariaciones.appendChild(opt);
                                }
                            });
                        }
                        resListProd.style.display = 'none'; recalcularTotales();
                    };
                    resListProd.appendChild(div);
                });
                resListProd.style.display = 'block';
            } else { 
                resListProd.innerHTML = `<div style="padding:15px; text-align:center;"><p style="font-size:12px;">Producto no existe.</p></div>`;
                resListProd.style.display = 'block'; 
            }
        });
    ...
    document.getElementById('form-crear-pedido-erp').addEventListener('submit', async (e) => {
        ...
        tbodyProd.querySelectorAll('tr').forEach(tr => {
            ...
            productosPedido.push({
                ...
                precio: parseFloat(precioBase + varInc)
            });
        });
        ...
        if (document.getElementById('erp-guardar-cliente').checked) {
            ...
            await window.supabase.from('usuarios').upsert({
                email: document.getElementById('erp-email').value || `cliente_${idPed}@cupissa.com`,
        ...
        const pedData = {
            idpedido: idPed,
            cliente: document.getElementById('erp-nombre').value,
            telefono: telPed ? parseInt(telPed) : null,
            usuario_email: document.getElementById('erp-email').value || null,
            direccion: document.getElementById('erp-direccion').value,
...
window.generarRemisionPDF = async function(idPedido) {
    ...
    const { data: prodsDB } = await window.supabase.from('pedidos_productos').select('*').eq('idpedido', idPedido);
    pedido.productos = prodsDB || [];
    ...
    doc.text(`Email: ${pedido.usuario_email || ''}`, 14, 79); doc.text(`Dirección: ${pedido.direccion || ''}`, 14, 85);
    ...
    window.mostrarToast(`PDF de la Remisión ${pedido.IDpedido} generado.`, "exito");
};